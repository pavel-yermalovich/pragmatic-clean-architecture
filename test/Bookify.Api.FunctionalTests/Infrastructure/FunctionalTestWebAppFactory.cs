...
public class FunctionalTestWebAppFactory : WebApplicationFactory<Program>, IAsyncLifetime
{
	...
	protected override void ConfigureWebHost(IWebHostBuilder builder)
	{
		builder.ConfigureTestServices(services =>
		{
			...
			{
				keycloakOptions.AdminUrl = $"{keycloakAddress}admin/realms/bookify/";
				keycloakOptions.TokenUrl = $"{keycloakAddress}realms/bookify/protocol/openid-connect/token";
			});

			services.Configure<AuthenticationOptions>(authenticationOptions =>
			{
				authenticationOptions.Issuer = $"{keycloakAddress}realms/bookify/";
				authenticationOptions.MetadataUrl = $"{keycloakAddress}realms/.well-known/openid-configuration";
			});
		});
	}
